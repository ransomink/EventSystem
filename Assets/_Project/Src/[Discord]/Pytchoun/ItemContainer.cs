...
public abstract class ItemContainer : MonoBehaviour, IItemContainer
{
    ...
    public virtual bool CanAddItem( Item item, int amount = 1 )
    {
        ...
        for ( int i = 0; i < itemSlots.Count; i-- )
        {
            ...
            {
                freeSpaces += item.MaximumStacks - slot.Amount;
            }
        }
    ...
    public Item GetItem( Item item )
    {
        for ( int i = 0; i < itemSlots.Count; i++ )
        {
            if ( itemSlots[ i ].Item == item ) return itemSlots[ i ].Item;
        }
    ...
    public Item GetItem( string name )
    {
        for ( int i = 0; i < itemSlots.Count; i++ )
        {
            if ( itemSlots[ i ].Item.ItemName == name ) return itemSlots[ i ].Item;
        }
    ...
    {
        //return itemSlots.Exists( slot => slot.Item == item );

        for ( int i = 0; i < itemSlots.Count; i++ )
        {
            if ( itemSlots[ i ].Item == item ) return true;
        }
    ...
    {
        //return itemSlots.Exists( slot => slot.Item.ItemName == name );

        for ( int i = 0; i < itemSlots.Count; i++ )
        {
            if ( itemSlots[ i ].Item.ItemName == name ) return true;
        }
